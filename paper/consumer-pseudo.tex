...
\INDENT {{\bf Function consume}():}
  \INDENT {{\bf if}(currentNode $\neq \bot$) {\bf then} \comment {common case}}
	...
	\ENDINDENT
	\INDENT {{\bf foreach} node $n$ in the pool {\bf do:} \comment {fair traversal}}
...
\INDENT {{\bf Function steal}(SCPool from):}
	...
	\INDENT {{\bf if} (newNode.idx+1 $=$ CHUNK\_SIZE)} 
	  \STATE chunkLists[steal].{\bf remove}(prevNode)
	  \STATE {\bf return} $\bot$
	\ENDINDENT
	...
	\INDENT {{\bf if} (task $=$ TAKEN $\vee$ \\
		\hspace{0.5cm} !CAS(c.tasks[idx+1], task, TAKEN)) {\bf then}} \label{alg:line:cas-steal} 
