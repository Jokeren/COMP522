...
\begin{enumerate}
	...
	We are especially interested in a management policy suitable for Non-Uniform Memory Access (NUMA) architectures (see Figure~\ref{fig:system-fig}), where each CPU has its own memory, such that accessing memory of other CPUs is executed via interconnect. As excessive rate of remote memory accesses can decrease the overall performance, it is highly desirable for an SCPool of a consumer to reside at the RAM close to its own CPU. 
\end{enumerate} 
...
A straightforward way to implement an API described above is to use a dynamic-size multi-producer multi-consumer FIFO queue (e.g., Michael-Scott queue).
In this case both produce() and produceForce() functions enqueue a new task to the given queue, while both consume() and steal() functions dequeue a task from the queue. While being conceptually simple, the performance of such FIFO implementation is far from being optimal. As we demonstrate in Section~\ref{sec:evaluation}, our SALSA algorithm improves its performance by an order of magnitude. 

The desired properties of any SCPool implementation are lock-freedom and starvation-freedom (every inserted task is eventually taken). A non-blocking FIFO queue clearly satisfies both of these properties; as we show in Section~\ref{sec:algo} SALSA does it as well. 
...
In the current work, we present a policy that exploits the locality properties of NUMA architectures and is aimed at achieving maximal throughput. If the individual SCPools themselves are lock-free and starvation-free, then our policy preserves these properties at the system level. 
\begin{itemize}
	\item {\bf Access order lists.} Each process in the system (producer or consumer) is provided with an ordered list of consumers, sorted according to their distance from that process (see Figure~\ref{fig:system-fig}). Intuitively, our policy is to have a producer mostly interact with the closest consumer, while stealing mainly happens inside the same processor node. 
	\item {\bf Producer's policy.} A producer inserting a task first calls the \emph{produce} function of the first SCPool in its access list. Note that a produce operation might fail if the pool is full, (which can be seen as evidence of that the corresponding consumer is overloaded).  In this case, the producer tries to insert the task into other pools, in the order defined by its access list. If all the pools are full, the producer invokes the \emph{produceForce} operation on the closest SCPool, which always succeeds (expanding the pool if needed). 
