...
for poolType in "NoFIFOCASPool" "LIFOPool" "NoFIFOPool" "MSQTaskPool" ; do
	for t in 1 2 4 8 16 32; do
		...
		for prodMigrate in "true" "false"; do		
			...
			echo "Affinity 1"
			for i in `seq 5`; do 
				perl runTest.pl --consumers=$t --producers=$t --poolType=$poolType --force="yes" --prodMigrate=$prodMigrate --affinity=$aff1 --affinityIdx="1" >> res_symmetric.csv
			done
			done
			...
			echo "Affinity 2"
			for i in `seq 5`; do 
				perl runTest.pl --consumers=$t --producers=$t --poolType=$poolType --force="yes" --prodMigrate=$prodMigrate --affinity=$aff2 --affinityIdx="2" >> res_symmetric.csv
			done
			done
			...
			echo "Affinity 3"
			for i in `seq 5`; do 
				perl runTest.pl --consumers=$t --producers=$t --poolType=$poolType --force="yes" --prodMigrate=$prodMigrate --affinity=$aff3 --affinityIdx="3" >> res_symmetric.csv
			done
			done
			...
			echo "Affinity 4"
			for i in `seq 5`; do 
				perl runTest.pl --consumers=$t --producers=$t --poolType=$poolType --force="yes" --prodMigrate=$prodMigrate --affinity=$aff4 --affinityIdx="4" >> res_symmetric.csv
			done
			done
		
			echo "no affinity"
			for i in `seq 5`; do 
				perl runTest.pl --consumers=$t --producers=$t --poolType=$poolType --force="no" --prodMigrate=$prodMigrate  --affinityIdx="0" >> res_symmetric.csv
			done
		done 
	done
done
