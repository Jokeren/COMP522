	...
	for p in 8; do
		for c in 24; do
			...
			for prodMigrate in "true" "false"; do
				...
				echo "Affinity 2"
				for i in `seq 5`; do 
					perl runTest.pl --consumers=$c --producers=$p --poolType=$poolType --force="yes" --prodMigrate=$prodMigrate --affinity=$aff2 --affinityIdx="6" >> res_assymetric.csv
				done
				done
				
				echo "no affinity"
				for i in `seq 5`; do 
					perl runTest.pl --consumers=$c --producers=$p --poolType=$poolType --force="no" --prodMigrate=$prodMigrate --affinityIdx="0" >> res_assymetric.csv
				done
			done
		done
	done
	...
	for p in 24; do
		for c in 8; do
			...
			for prodMigrate in "true" "false"; do
				...
				echo "Affinity 3"
				for i in `seq 5`; do 
					perl runTest.pl --consumers=$c --producers=$p --poolType=$poolType --force="yes" --prodMigrate=$prodMigrate --affinity=$aff3 --affinityIdx="7" >> res_assymetric.csv
				done
				done
				...
				echo "Affinity 4"
				for i in `seq 5`; do 
					perl runTest.pl --consumers=$c --producers=$p --poolType=$poolType --force="yes" --prodMigrate=$prodMigrate --affinity=$aff4 --affinityIdx="8" >> res_assymetric.csv
				done
				done
